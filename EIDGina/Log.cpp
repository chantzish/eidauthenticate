...
bool Log::FormatLogEntry(wchar_t* msg, int cch, const wchar_t* sourceFile, int lineNumber, const wchar_t* formatString, va_list args) {
    ...
    HRESULT hr = swprintf_s(msg, cch, L"%s %d: %02d:%02d:%02d:%04d %s(%d): ",
        tssDesc,
        tsSessionId,
        systemTime.wHour,
        systemTime.wMinute,
    ...
    wchar_t suffix[256];
    hr = swprintf_s(suffix, sizeof suffix / sizeof *suffix, formatString, args);
    if (FAILED(hr)) return false;

    hr = wcscat_s(msg, cch, suffix);
    if (FAILED(hr)) return false;

    hr = wcscat_s(msg, cch, L"\r\n");
    if (FAILED(hr)) return false;
