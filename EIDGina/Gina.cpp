...
// to interactively debug, be sure _DEBUG is defined, and pass NULL for both hWlx and pWinlogonFunctions
	...
	GuiHelper::EnableWindowsXPTheme();
...
Gina::Gina(IWinLogon* pWinLogon, HANDLE hLsa)
  : _pWinLogon(pWinLogon), _hLsa(hLsa), _hToken(0), _profilePath(0), _pStatusWindow(0) {
	...
	// Winlogon doesn't send any notifications. Disabling, to avoid any surprise
	_pWinLogon->wlxSetOption(WLX_OPTION_USE_SMART_CARD, FALSE, &oldValue);
	sendSASNotification = false;
...
int Gina::LoggedOutSAS(DWORD dwSasType, PLUID pAuthenticationId, PSID pLogonSid, PDWORD pdwOptions,
    ...
    *pdwOptions = 0; // we always let WinLogon load the user profile for us
	...
	else if (dwSasType == WLX_SAS_TYPE_SC_INSERT)
	{
        ...
        if (!SecurityHelper::CallLsaLogonUserEIDAuthenticate(_hLsa,
												dlgPin.certificate,
                                                dlgPin.pin,
                                                Interactive,
...
void Gina::Callback(EID_CREDENTIAL_PROVIDER_READER_STATE Message, __in LPCTSTR szReader,__in_opt LPCTSTR szCardName, __in_opt USHORT ActivityCount) {
	switch(Message)
	{
